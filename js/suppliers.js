...
window.Suppliers = (function () {
  ...
  // Load rows (approved only), from sessionStorage cache or the live proxy.
  ...
  function load(onProgress, force) {
    ...
    if (!force) {
      try {
        ...
        if (cached) { rows = JSON.parse(cached); return Promise.resolve(rows); }
      } catch (e) { /* ignore */ }
    }

    inflight = fetchAll(onProgress).then(function (all) {
  ...
  function init() {
    ...
    function render() {
      ...
      setStatus('Showing ' + shown.length + (total > shown.length ? ' of ' + total : '') +
        ' match' + (total === 1 ? '' : 'es') + ' · ' + Suppliers.count() + ' records loaded (cheapest first).');
    }

    var started = false;
    function ensureLoaded(force) {
      started = true;
      setStatus(force ? 'Refreshing supplier prices…' : 'Loading live supplier prices… (first load can take ~30–60s if the source is asleep)');
